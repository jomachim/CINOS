...
class GameStats{// extends h2d.Object
    ...
    public static function save(){
        // make an array of name and done to hxd.save serialization
        var tmp=[];
        for(a in ALL){
            tmp.push({name:a.name,done:a.done,data:a.data});
        }
        return tmp;
    }
    public function load(s){
        ...
        clearAll();
        for(i in 0...s.length){
            var a=new Achievement(s[i].name,s[i].name,()->true,()->true,s[i].data);
            a.done=true;
            registerState(a);
        }
        updateAll();
...
class Achievement {
    ...
    public var marks:Dynamic=null;//tools.MarkerMap<Types.LevelMark>;
    public var done:Bool=false; 
    public function new(_name:String,_state:String,?_success:()->Bool,?_cb:()->Void,_data:Dynamic=null,?_marks:Dynamic=null){//tools.MarkerMap<Types.LevelMark>
            name=_name;
            state=_state;
            success=_success;
            cb=_cb;
